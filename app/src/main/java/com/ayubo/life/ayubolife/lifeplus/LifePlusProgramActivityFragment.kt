...
import kotlinx.android.synthetic.main.activity_life_plus_program.view.*
...
class LifePlusProgramActivityFragment : Fragment(), GoogleApiClient.ConnectionCallbacks,
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.activity_life_plus_program, container, false);
        ...
        if (Constants.type == Constants.Type.LIFEPLUS) {
            view.btn_programs_img.setBackgroundResource(R.drawable.new_like_active_lifeplus);
        }
        ...
        if (googleSupportServices.isGooglePlayServicesAvailable) {
            ...
            if (requestCode == REQUEST_LOGIN_CODE) {
                ...
                if (huaweiIdAuthManager.isSuccessful) {
                    ...
                    Toast.makeText(context, "Authorization was not granted", Toast.LENGTH_SHORT)
                        .show();
                }
...
//                                showAlert_Add(it, dataObj)
    ...
    fun deleteAPost(postobj: Post) {
        ...
        val call: Call<com.ayubo.life.ayubolife.pojo.timeline.LikeMainResponse> =
            apiService.DeleteAPost(
                AppConfig.APP_BRANDING_ID,
                appToken,
                Integer.toString(postobj.getPostId())
            );

        call.enqueue(object : Callback<com.ayubo.life.ayubolife.pojo.timeline.LikeMainResponse> {
            override fun onResponse(
    ...
    fun checkAndAuthorizeHealthApp() {
        ...
        val mSettingControllerQueryTask: Task<Boolean> = mSettingController.healthAppAuthorization;
    ...
    fun connectHuaweiSignIn() {
        ...
        val huaweiAuthService: HuaweiIdAuthService =
            HuaweiIdAuthManager.getService(getContext(), authParams);
...
//                task.execute();
            ...
            requireActivity().startActivityForResult(
                huaweiAuthService.getSignInIntent(),
                REQUEST_LOGIN_CODE
            );
        }
        ...
        authHuaweiIdTask.addOnFailureListener {
            ...
            val signInIntent: Intent = huaweiAuthService.getSignInIntent();
...
//        task.execute();
    ...
    private fun openGoogleFitInPlayStore() {
        ...
        if (Constants.type == Constants.Type.AYUBO) {
            layoutView.findViewById<TextView>(R.id.lbl_app_version).text =
                "AyuboLife needs to fetch your step count from GoogleFit and other Google services."
        }
    ...
    fun sendLatestStepsToServer(stps: String) {
        ...
        val task: sendAyuboStepsToServer_ForTenDays0 = sendAyuboStepsToServer_ForTenDays0();
        task.execute();
    }
...
//    internal inner class newServiceAPIFor0DaysGFIT : AsyncTask<List<StepObj>, Void, Void>() {
//        override fun doInBackground(vararg arrayLists: List<StepObj>?): Void? {
//            val stepDetail: List<StepObj>? = arrayLists[0];
...
//            val str_currentTime : String = currentTime.toString();
//            pref.setLastDataUpdateTimestamp(str_currentTime);
//        }
//    }

//    private fun saveWalkWinDailySteps( obj : List<StepObj>) {
