package com.ayubo.life.ayubolife;

import android.content.BroadcastReceiver;
import android.content.Context;
import android.content.Intent;
import android.os.Bundle;
import android.telephony.SmsMessage;
import android.util.Log;

import com.ayubo.life.ayubolife.login.LetsGo;
import com.ayubo.life.ayubolife.mobile_login.Config;
import com.ayubo.life.ayubolife.mobile_login.SmsListener;

/**
 * Created by Ravi on 09/07/15.
 */
public class SmsReceiver extends BroadcastReceiver {
    private static final String TAG = SmsReceiver.class.getSimpleName();
    private static SmsListener mListener;


    @Override
    public void onReceive(Context context, Intent intent) {
        Log.e(TAG, "SMS =========================================");
        Log.e(TAG, "SMS =========================================");
        Log.e(TAG, "SMS =========================================");
        Log.e(TAG, "SMS =========================================");
        Log.e(TAG, "SMS =========================================");
        Log.e(TAG, "SMS =========================================");

        final Bundle bundle = intent.getExtras();
        try {
            if (bundle != null) {
                Object[] pdusObj = (Object[]) bundle.get("pdus");
                for (Object aPdusObj : pdusObj) {
                    SmsMessage currentMessage = SmsMessage.createFromPdu((byte[]) aPdusObj);
                    String senderAddress = currentMessage.getDisplayOriginatingAddress();
                    String message = currentMessage.getDisplayMessageBody();

                    Log.e(TAG, "Received SMS: " + message + ", Sender: " + senderAddress);

                    String verificationCode = getVerificationCode(message);

                    Log.e(TAG, "OTP received: " + verificationCode);

                //    mListener.messageReceived(verificationCode);

                    // if the SMS is not from our gateway, ignore the message


                    // verification code from sms

                    if(senderAddress.equals("HEMAS")){
                        Intent hhtpIntent = new Intent(context, LetsGo.class);
                        hhtpIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        hhtpIntent.putExtra("mySms", message);
                        context.startActivity(hhtpIntent);
                    }else{

                    }

                  //  Intent hhtpIntent = new Intent(context, HttpService.class);
                 //   hhtpIntent.putExtra("otp", verificationCode);
                  //  context.startService(hhtpIntent);

                    if (!senderAddress.toLowerCase().contains(Config.SMS_ORIGIN.toLowerCase())) {
                        Log.e(TAG, "SMS is not for our app!");
                        return;
                    }

//                    Intent smsShow = new Intent(context, Mobile_Login_Activity.class);
//                    smsShow.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                    smsShow.putExtra("mySms", verificationCode);
//                    context.startActivity(smsShow);

                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Exception: " + e.getMessage());
        }
    }
    public static void bindListener(SmsListener listener) {
        mListener = listener;
    }
    /**
     * Getting the OTP from sms message body
     * ':' is the separator of OTP from the message
     *
     * @param message
     * @return
     */
    private String getVerificationCode(String message) {
        String code = null;
        int index = message.indexOf(Config.OTP_DELIMITER);

        if (index != -1) {
            int start = index + 2;
            int length = 6;
            code = message.substring(start, start + length);
            return code;
        }

        return code;
    }
}
