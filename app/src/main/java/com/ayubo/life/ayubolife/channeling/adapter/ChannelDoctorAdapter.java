    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (sessions.get(position) instanceof Session) {
            ...
            if (holder instanceof SessionViewHolder) {
                ...
                if (sessionDate != null) {
                    sessionViewHolder.txtDate.setText(TimeFormatter.getDay(activity, sessionDate.getTime()));
                    String weekDay = TimeFormatter.millisecondsToString(sessionDate.getTime(), TimeFormatter.TIME_FORMAT_WEEK_SHORT);

                    String timeSlot = AppHandler.convertHourFormat(session.getTime());
                    sessionViewHolder.txtTime.setText(String.format("%s %s", weekDay, timeSlot));
                }
            ...
            Glide.with(activity).load(source.getLogo()).into(sourceViewHolder.imgSource);
